...
The REST API documentation can be found on [developers.dwolla.com](https://developers.dwolla.com). The full API of this library can be found in [api.md](api.md).
...
async function main() {
  const response = await client.tokens.generate({ grant_type: 'REPLACE_ME' });
...
async function main() {
  const params: Dwolla.TokenGenerateParams = { grant_type: 'REPLACE_ME' };
  const response: Dwolla.TokenGenerateResponse = await client.tokens.generate(params);
}
...
```ts
async function main() {
  const response = await client.tokens.generate({ grant_type: 'REPLACE_ME' }).catch(async (err) => {
    if (err instanceof Dwolla.APIError) {
...
await client.tokens.generate({ grant_type: 'REPLACE_ME' }, {
  maxRetries: 5,
});
```
...
const response = await client.tokens.generate({ grant_type: 'REPLACE_ME' }).asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: response, response: raw } = await client.tokens
  .generate({ grant_type: 'REPLACE_ME' })
  .withResponse();
...
add the following import before your first import `from "Dwolla"`:
...
- Deno v1.28.0 or higher, using `import Dwolla from "npm:dwolla-stainless-node"`.
- Bun 1.0 or later.
- Cloudflare Workers.
- Vercel Edge Runtime.
