...
const validatePositiveInteger = (name: string, n: unknown): number => {
  if (typeof n !== 'number' || !Number.isInteger(n)) {
    throw new DwollaError(`${name} must be an integer`);
  }
  if (n < 0) {
    throw new DwollaError(`${name} must be a positive integer`);
  }
  return n;
};
...
export const ensurePresent = <T>(value: T | null | undefined): T => {
  if (value == null) throw new DwollaError(`Expected a value to be given but received ${value} instead.`);
  return value;
};
...
export const coerceInteger = (value: unknown): number => {
  ...
  throw new DwollaError(`Could not coerce ${value} (type: ${typeof value}) into a number`);
};
...
export function debug(action: string, ...args: any[]) {
  if (typeof process !== 'undefined' && process?.env?.['DEBUG'] === 'true') {
    console.log(`Dwolla:DEBUG:${action}`, ...args);
  }
}
...
export const toBase64 = (str: string | null | undefined): string => {
  ...
  throw new DwollaError('Cannot generate b64 string; Expected `Buffer` or `btoa` to be defined');
};
